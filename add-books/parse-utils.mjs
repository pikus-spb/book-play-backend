...
import { cleanupSpaces } from './clean-utils.js'

const { JSDOM } = jsdom;
const IMAGE_SIZE_MAX = 1000000;

export const getFileEncoding = (file) => {
    ...
    return new Promise((resolve, reject) => {
        fs.readFile(file, null, (err, data) => {
            if (!err) {
                ...
                if (authorFirstName) {
                    authorFirstName = cleanupSpaces(authorFirstName.innerHTML);
                }
                let authorLastName = document.querySelector('author last-name');
                if (authorLastName) {
                    authorLastName = cleanupSpaces(authorLastName.innerHTML);
                }

                const logo = parseBookLogo(data);
                const title = cleanupSpaces(document.querySelector('book-title').innerHTML);
...
export const parseBookLogo = (data) => {
    const dom = new JSDOM("");
