export class DbUtils {
    addBookToDataBase(pool, bookData) {
        ...
        return new Promise((resolve, reject) => {
            ...
            pool.query(
                ...
                [authorFirstName, authorLastName, title, `${authorFirstName} ${authorLastName} - ${title}`, this.getBase64ImageData(logo), content],
                (err, rows) => {
                    if (err) {
                        reject(err);
                    } else {
